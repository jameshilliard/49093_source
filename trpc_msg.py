...
#
name_from_serviceID = dict(zip(service_formats.keys(), 
        (f.name for f in service_formats.values())))

#*****************************************************************************
# Lookup service names from service IDs.  This is the reverse of
# name_from_serviceID.
#
serviceID_from_name = dict(zip(name_from_serviceID.values(), name_from_serviceID.keys()))
...
#
name_from_methodID = dict(zip(method_formats.keys(), 
        (f.name for f in method_formats.values())))

#*****************************************************************************
# Lookup method names from method IDs.  This is the reverse of
# name_from_methodID.
#
methodID_from_name = dict(zip(name_from_methodID.values(), name_from_methodID.keys()))
...
class TrpcPacket:
    ...
    def from_rx_packet(pck_str):
        """ Create a TrpcPacket from a packet string (as it would be received
            ...
            """
        p = packet.Packet.from_str(pck_str)
        if p.type != packet.TYPE_TRPC:
            return None
    ...
    #*************************************************************************
    def __str__(self):
        """ String representation of the packet.  This is as it appears in
            current tRPC log files.  See trpclog.py for more info.
            """
